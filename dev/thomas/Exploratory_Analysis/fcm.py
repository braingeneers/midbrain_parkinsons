...
#     firing_rates = calculate_mean_firing_rates(sub_start)
    ...
    def sttc_neuron_plotter(inp_sd, upd_node_info, line_threshold):
        ...
        neuron_amp = calculate_mean_firing_rates(inp_sd)
        
        print(len(neuron_x), len(neuron_amp))

        plt.figure(figsize=(8, 6))
        savedFCMplot = plt.scatter(neuron_x, neuron_y, s=5*neuron_amp, c=upd_node_info)
        ...
        node_degree_legend = plt.legend(handles=node_degree_legend_elements, title='Firing Rate (Hz)', loc='lower right')
